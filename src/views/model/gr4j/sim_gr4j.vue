      ...
      >
        ...
        <el-form-item label="参数优选算法" prop="calib">
          <el-select
            ...
            @change="getParams"
          >
            <el-option label="NSGAII" value="NSGAII"></el-option>
            <el-option label="GM" value="GM"></el-option>
            <el-option label="MOPSO" value="MOPSO"></el-option>
            <el-option label="SCEUA" value="SCEUA"></el-option>
          </el-select>
        </el-form-item>
        <el-form-item label="目标函数" prop="aim">
          <el-select
            class="input-unit"
            v-model="dataForm.aim"
            placeholder="请选择"
            @change="getParams"
          >
            <el-option label="Rp" value="Rp"></el-option>
            <el-option label="Rv" value="Rv"></el-option>
...
import { childPlanList } from "@/api/hydromodel/gr4j_multi";
...
export default {
  ...
  methods: {
    ...
    //选取子流域列表
    getUpChildBasininfo() {
      getChildBasin({ basin_id: this.currentsk })
        .then(res => {
          this.childbasin_form = res.data;
        })
        .catch(res => {
          this.$messages({
            type: "warning",
    ...
    // 获取设备列表
    getDevice() {
      deviceList({ basin_child_id: this.childcurrentsk }).then(({ data }) => {
    ...
    getList() {
      ...
      childPlanList({ basin_child_id: this.childcurrentsk }).then(
        ({ data }) => {
          this.list = data;
        }
      );
    ...
    getParams() {
      ...
      return new Promise((resolve, reject) => {
        calibrateList({ basin_child_id: this.childcurrentsk }).then(
          ({ data }) => {
            this.paramList = data;
    ...
    getParamsman() {
      ...
      this.resetFormman();
      return new Promise((resolve, reject) => {
        calibmanList({ basin_child_id: this.childcurrentsk }).then(
          ({ data }) => {
            this.paramList = data;
    ...
    // 根据设备和时间获取数据
    handleData() {
      this.dataQuery.basin_child_id = this.childcurrentsk;
    ...
    handleSubmit() {
      ...
      this.$refs["dataForm"].validate(valid => {
        if (valid) {
          this.dataForm.basin_child_id = this.childcurrentsk;
    ...
    handleSubmitman() {
      ...
      this.$refs["dataForm"].validate(valid => {
        if (valid) {
          this.dataFormman.basin_child_id = this.childcurrentsk;
