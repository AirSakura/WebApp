<template>
  <div class="app-container">
    <div style="display: flex">
      <div class="flexPart" style="width:300px">
        <div
          class="dm-container"
      ...
      <div class="flexMain" style="width:calc(100% - 310px)">
        <div
    ...
    >
      <div style="display: flex;margin-bottom: 10px;">
        <p class="filter-line" style="margin-right: 5px;">
          <el-select
      ...
      <el-divider content-position="center">已选数据</el-divider>
      <div style="display: flex;">
        <el-table
          :data="timeList"
          element-loading-text="正在查询中。。。"
      ...
      <div slot="footer" class="dialog-footer">
        ...
        <el-button type="primary" round @click="saveModel">确定</el-button>
      </div>
    </el-dialog> -->
...
export default {
  ...
  methods: {
    ...
    // 获取洪水场次列表
    getTimeList() {
      userModelInfo({ partid: this.currentsk }).then(({ data }) => {
        this.timeList = data;
      });
    },
    // 开启动率定/验证弹窗
    ...
    saveCalibrate(type) {
      calibrate({
        ...
        this.$messages({
          type: "success",
          message:
            type == 0 ? "模型计算中，请耐心等待！" : "模型校验中，请耐心等待！"
        });
    ...
    // 获取设备列表
    getDevice() {
      deviceList({ partid: this.currentsk }).then(({ data }) => {
    ...
    // 获取方案列表
    getList() {
      planList({ partid: this.currentsk }).then(({ data }) => {
        this.list = data;
      });
    },
    // 根据模型获取参数
    getParams() {
      ...
      return new Promise((resolve, reject) => {
        calibrateList({ type: obj.model_name }).then(({ data }) => {
          this.paramList = data;
    ...
    // 根据设备和时间获取数据
    handleData() {
      this.dataQuery.partid = this.currentsk;
    ...
    handleSubmit() {
      ...
      this.$refs["dataForm"].validate(valid => {
        if (valid) {
          this.dataForm.partid = this.currentsk;
