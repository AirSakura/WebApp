...
import { roletree } from '@/api/query'
...
export default {
  ...
  methods: {
    ...
    addPlaceHolder(value) {
      const placeholder = this.$el.querySelector('.vue-treeselect__placeholder')
    ...
    // 增加拖拽下拉功能
    open(instanceId) {
      const dom = document.querySelector(`.vue-treeselect[data-instance-id='${instanceId}']`)
      let listDom
      this.$nextTick(() => {
